...
router.beforeEach((to, from, next) => {
    ...
    if (store.getters.token) { // 判断是否有token
        if (to.path === '/login') {
            ...
            if (store.getters.roles.length === 0) { // 判断当前用户是否已拉取完user_info信息 角色信息为0 则判定为用户未拉取信息
                store.dispatch('GetInfo').then(res => { //  并且授权登录
                    ...
                    console.log('ssssssssssssssssssssssss--->' + Cookies.get('JSESSIONID'));
                    console.log('前端token(授权码) 信息------>' + store.getters.token);
                    next({ path: '/' });
                });
            } else {
                console.log('aaaaa999');
