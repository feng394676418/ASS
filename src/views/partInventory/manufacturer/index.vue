...
<div class="app-container calendar-list-container page-content">
  <div class="Sharing_head">
                ...
                <div class="page-header-inner">
                  <div class="search-form search-form-expanded">
                    <div class="input-group">
                      ...
                      <input type="text" class="form-control" v-model="listQuery.searchValue" maxlength="" :placeholder="$t('inventory.entershipper')" name="number" @keydown="handleSerachEnter($event)"></div>
                  </div>                 
    ...
    <div id="top_menu">
	            ...
	            <div class="order_list_menu ">
	            	<div class="pull-right order_list_top">
                        <form>	                 
                            <el-select clearable v-model="listQuery.searchCode" :placeholder="$t('inventory.provider')" @change="handlesearchByProviderCode" size="small">
                                <el-option
    ...
    <el-table ref="partInventoryTable"  :data="list" v-loading.body="listLoading"  style="margin-top:5px;width: 100%" @sort-change="handTableSort" @selection-change="handleSelectionChange">
      ...
      <el-table-column  min-width="15%" align="center" :label="$t('inventory.provider')" prop="owner" sortable="custom">
      ...
      <el-table-column min-width="22%" align="center" :label="$t('inventory.name')">
        <template scope="scope">
          <span>{{scope.row.enName}}</span>
        </template>
      </el-table-column>

      <el-table-column min-width="8%" align="center" :label="$t('inventory.unit')">
        <template scope="scope">
          <span>{{scope.row.unit}}</span>
        </template>
      </el-table-column>

      <el-table-column min-width="15%" align="center" :label="$t('inventory.qty')">
        <template scope="scope">
...
export default {
    ...
    methods: {
        ...
        handleViewInventoryChange(sku,providerCode,owner,inventoryType){
            ...
            this.$router.push({ path: '/partInventory/manufacturer/change/index/'+sku+"/"+owner+"/"+providerCode+"/"+inventoryType });
        },
        getProviderList() {
              // 获取货主对应的 服务商list
