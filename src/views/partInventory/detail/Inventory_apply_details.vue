	            			...
	            			<div class="scroll-box">
									...
									<template v-for="(logInfo,index) in logInfoList">
										<showButtonChild :logInfo="logInfo" :key="index"></showButtonChild>
											...
											<!--厂商审核按钮-->
									<template v-if="lastLogStatus === 11">
											<reviewButtonChild :baseInfo="baseInfo" v-on:listenBaseInfo="refreshBaseInfo"></reviewButtonChild>
									</template>

										<!--厂商发货按钮-->
									<template v-if="lastLogStatus === 12">
											<deliveryButtonChild :baseInfo="baseInfo" v-on:listenBaseInfo="refreshBaseInfo"></deliveryButtonChild>
										</template>
...
import reviewButtonChild from './reviewButtonChild';
import deliveryButtonChild from './deliveryButtonChild';
...
export default {
  ...
  props: {},
  components: { rightButtonChild, showButtonChild, reviewButtonChild, deliveryButtonChild },
  data() {
      return {
				baseInfo: {},
				searchValue: '',
  ...
  methods: {
			...
			getLogDetail(this.stockOrderNumber).then(response => {
					if (response.data.status === '0') {
						...
						this.logInfoList.forEach((item, index) => {
									...
									if (index === this.logInfoList.length - 1 && item.logStatus !== 12 && this.logInfoList.length > 1) {
										this.lastLogStatus = item.logStatus;
										this.$set(item, 'className', progressSTS[3]);
										this.$set(item, 'cssStyle', 'visibility:hidden');
									} else if (index === this.logInfoList.length - 1 && item.logStatus === 12 && this.logInfoList.length > 1) {
										this.lastLogStatus = item.logStatus;
									}
						});
					}
	...
	created() {
		this.searchLogInfoList();
		this.getBaseInfo();
		$('#nav9').next().addClass('is-active');
		$('#nav9').next().children().find('li').eq(2).addClass('is-active');
	},
		beforeDestroy() {
		$('#nav9').next().removeClass('is-active');
