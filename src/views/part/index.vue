<template>
  <div class="app-container calendar-list-container page-content">
    ...
    <el-table ref="orderTable" :data="list" v-loading.body="listLoading" border fit highlight-current-row style="margin-top:5px;width: 100%" @sort-change="handTableSort" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column align="center" :label="$t('stockOrder.detail.serialNo')" width="100">
        <template scope="scope">
          <span>{{scope.$index+1+(listQuery.pageNum-1)*listQuery.pageSize}}</span>
        </template>
      </el-table-column> -->

      <!-- <el-table-column  align="center" :label="$t('Warehouse.Shipper')" prop="refNumber" sortable="custom" width="100">
        <template scope="scope">
          <span>{{scope.row.owner}}</span>
        </template>
      </el-table-column> -->

      <el-table-column  :label="$t('part.coding')" prop="trackingno" sortable="custom" min-width="15%">
        <template scope="scope">
          <span>{{scope.row.sku}}</span>
        </template>
      </el-table-column>

      <el-table-column  align="center" :label="$t('Warehouse.Chinesename')" min-width="22%">
        <template scope="scope">
          <span>{{scope.row.cnName}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('Warehouse.Englishname')" min-width="22%">
        <template scope="scope">
          <span>{{scope.row.enName}}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('part.unit')" min-width="10%">
        <template scope="scope">
          <span>{{scope.row.unit}}</span>
        </template>
      </el-table-column>
<!-- 
       <el-table-column  align="center" :label="$t('order.Detail.Price')" width="100">
        <template scope="scope">
          <span>{{scope.row.price}}</span>
        </template>
      </el-table-column>

      <el-table-column  align="center" :label="$t('part.outPrice')" width="150">
        <template scope="scope">
          <span>{{scope.row.outPrice}}</span>
        </template>
      </el-table-column> -->

      <!-- <el-table-column align="center" :label="$t('part.Tagcode')" width="100">
        <template scope="scope">
          <span>{{scope.row.labelCode}}</span>
        </template>
      </el-table-column> -->

      <el-table-column align="center" :label="$t('Warehouse.status')" min-width="10%">
        <template scope="scope">
          <el-tag :type="scope.row.status | statusFilter">{{scope.row.status | statusNameFilter}}</el-tag>
        </template>
      </el-table-column>

      <el-table-column  align="center" :label="$t('part.suitModel')" min-width="21%">
        <template scope="scope">
      ...
      <el-table-column align="center" :label="$t('Warehouse.Modifypeople')" min-width="15%">
        <template scope="scope">
            ...
            <div class="order_detection main_form_input">
              ...
              <form role="form" ref="partInfoForm">
                <div class="form-group col-md-6">
                  ...
                  <input class="form-control" id="sku" ref="sku" placeholder="" alt="verifyPartForm" v-verify-input:nonvoid ="{id:'coding',format:true,title:$t('part.coding')}" type="text" v-model="part.sku">
                  <div v-verify-msg:coding></div>
                </div>
                <div class="form-group col-md-6">
                  <label for="">{{$t('Warehouse.Chinesename')}}<b>*</b></label>
                  <input class="form-control" id="cnName" placeholder="" alt="verifyPartForm" v-verify-input:nonvoid ="{id:'cnName',format:true,title:$t('Warehouse.Chinesename')}" type="text" v-model="part.cnName">
                  <div v-verify-msg:cnName></div>
                </div>

                <div class="form-group col-md-6">
                  <label for="">{{$t('Warehouse.Englishname')}}<b>*</b></label>
                  <input class="form-control" id="enName" placeholder="" alt="verifyPartForm" v-verify-input:nonvoid ="{id:'enName',format:true,title:$t('Warehouse.Englishname')}" type="text" v-model="part.enName">
                  <div v-verify-msg:enName></div>
                </div>
                <div class="form-group col-md-6">
                  <label for="">{{$t('part.unit')}}<b>*</b></label>
                  <input class="form-control" id="unit" placeholder="" alt="verifyPartForm" v-verify-input:nonvoid ="{id:'unit',format:true,title:$t('part.unit')}" type="text" v-model="part.unit">
                  <div v-verify-msg:unit></div>
                </div>
                <!-- <div class="form-group col-md-6">
                  <label for="">{{$t('part.price')}}<b>*</b></label>
                  <input class="form-control" id="price" placeholder="" v-verify-input:nonvoid ="{id:'price',format:true,title:$t('part.price')}" type="text" v-model="part.price">
                  <div v-verify-msg:price></div>
                </div>
                <div class="form-group col-md-6">
                  <label for="">{{$t('part.outPrice')}}<b>*</b></label>
                  <input class="form-control" id="outPrice" placeholder="" v-verify-input:nonvoid ="{id:'outPrice',format:true,title:$t('part.outPrice')}" type="text" v-model="part.outPrice">
                  <div v-verify-msg:outPrice></div>
                </div> -->
                <div class="form-group col-md-12">
                  <label for="">{{$t('part.suitModel')}}<b>*</b></label>
                  <input  class="form-control" id="suitModel" placeholder="" alt="verifyPartForm" v-verify-input:nonvoid ="{id:'suitModel',format:true,title:$t('part.suitModel')}" v-model="part.suitModel"></el-input>
...
<script>
  ...
  import Vue from 'vue';
  import { getPartList,addPart,getPartById,updatePart,invalidPart,validPart,getPartExcelUrl } from 'api/partManage';
